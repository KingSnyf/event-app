...
function ajouterEventsDemo(): void {
  ...
  const demoEvents = [
    ...
    new Event(
      "Tournoi de Football", 
      "Championnat inter-universitaire de football", 
      new Date("2025-03-20"), 
      "Stade Municipal", 
      "sport", 
      50
    ...
    new Event(
      ...
      "Apprenez TypeScript, HTML et CSS", 
      new Date("2025-02-10"), 
      "Salle Informatique", 
      "atelier", 
      30
    ...
    new Event(
      ...
      "Soirée musicale avec artistes locaux", 
      new Date("2025-12-20"), 
      "Centre Culturel", 
      "autre", 
      200
...
function afficherEvents(events?: Event[]): void {
...
function afficherDetailEvent(eventId: number): void {
...
function gererCreationEvent(e: SubmitEvent): void {
  e.preventDefault();
  
  const titre = (document.getElementById("event-titre") as HTMLInputElement).value.trim();
  const desc = (document.getElementById("event-description") as HTMLTextAreaElement).value.trim();
  const dateStr = (document.getElementById("event-date") as HTMLInputElement).value;
  const lieu = (document.getElementById("event-lieu") as HTMLInputElement).value.trim();
  const cat = (document.getElementById("event-categorie") as HTMLSelectElement).value as EventCategory;
  const cap = parseInt((document.getElementById("event-capacite") as HTMLInputElement).value);
...
function filtrerParCategorie(): void {
  const select = document.getElementById("filter-categorie") as HTMLSelectElement;
  if (!select) return;
  ...
  if (categorie === "toutes") {
    ...
    const eventsFiltres = eventManager.filtrerParCategorie(categorie as EventCategory);
    afficherEvents(eventsFiltres);
  }
}


(window as any).afficherDetailEvent = afficherDetailEvent;
(window as any).fermerModal = fermerModal;
